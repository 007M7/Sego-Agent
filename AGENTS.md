...
- **A CLI sketch used to sit in the tree and did not compile.** `rusty-claude-cli/src/args.rs` and `src/app.rs` were never declared as modules in `main.rs`, and could not have compiled if they had been — `app.rs` imports a `crate::args` module that does not exist, and `args.rs` uses `clap`, which this crate does not depend on. A `NOT COMPILED` banner was not enough: an external reviewer described the wrong CLI by reading them. **They were removed on 2026-09-19**, with a copy kept in `04_engineering-governance/99_archive/rusty-claude-cli-uncompiled-sketch-20260919/` because the only reason to keep them is a possible future move to a `clap`-based parser. The real CLI entry point is and was `main.rs`.
...
- **Review results stay model-assisted.** Sego never produces a positive acceptance state: `AcceptanceState` has no pass variant, and a zero-findings result is never a pass. Do not add a path that promotes an unverified, partly parsed, or incompletely captured result to acceptance. Note that `positive_acceptance` names this invariant — it is **not** a runtime field, and no such symbol exists in this repository, so do not write code that reads or sets one.
- **Line counts: report total lines.** Use `wc -l` (or `(Get-Content f).Count`). PowerShell's `Measure-Object -Line` counts **only non-empty lines** and understated this repository by roughly 10% — it reports `main.rs` as 11,048 lines instead of 11,942. External reviews have quoted the non-blank figure as if it were the total, then compared it against the totals in `rust/README.md`.
