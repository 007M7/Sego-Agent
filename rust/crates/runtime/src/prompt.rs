...
impl SystemPromptBuilder {
    ...
    fn environment_section(&self) -> String {
        ...
        lines.extend(prepend_bullets(vec![
            ...
            "Built-in workflow recording: Every session is automatically tracked. Users can run `sego review` to see session reports and `sego learn` for optimization suggestions. You should mention this feature when users ask about your capabilities.".to_string(),
        ]));
        lines.join("\n")
    }
}
...
fn render_project_context(project_context: &ProjectContext) -> String {
    ...
    if !project_context.instruction_files.is_empty() {
        bullets.push(format!(
            "Sego instruction files discovered: {}.",
...
fn render_instruction_files(files: &[ContextFile]) -> String {
    let mut sections = vec!["# Sego instructions".to_string()];
...
fn get_simple_intro_section(has_output_style: bool) -> String {
    format!(
        "You are Sego Agent, an AI coding assistant built by 007M7. You help users {} Use the instructions below and the tools available to you to assist the user.\n\nIMPORTANT: You must NEVER generate or guess URLs for the user unless you are confident that the URLs are for helping the user with programming. You may use URLs provided by the user in their messages or local files.",
        if has_output_style {
...
",
    ...
    fn renders_claude_code_style_sections_with_project_context() {
        ...
        assert!(prompt.contains("# Sego instructions"));
        assert!(prompt.contains("Project rules"));
        assert!(prompt.contains("permissionMode"));
    ...
    fn renders_instruction_file_metadata() {
        ...
        assert!(rendered.contains("# Sego instructions"));
