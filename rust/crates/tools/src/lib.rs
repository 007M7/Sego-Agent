...
impl GlobalToolRegistry {
    ...
    #[must_use]
    pub fn extension_tool_names(&self) -> BTreeSet<String> {
        self.runtime_tools
            .iter()
                ...
                match event {
                    ...
                    ApiStreamEvent::ContentBlockDelta(delta) => match delta.delta {
                        ...
                        ContentBlockDelta::ThinkingDelta { thinking } => {
                            if let Some((accumulated, _)) = pending_thinking.get_mut(&delta.index) {
...
fn execute_repl(input: ReplInput) -> Result<ReplOutput, String> {
    ...
    let started = Instant::now();
    let mut process = Command::new(runtime.program.as_ref());
    process
        .args(runtime.args)
        .arg(&input.code)
...
mod tests {
  ...
  "nbformat_minor": 5
    ...
    fn bash_tool_reports_success_exit_failure_timeout_and_background() {
        ...
        let failure =
            execute_tool("bash", &json!({ "command": shell_stderr_exit_command("oops", 7) }))
