//! The MCP tool domain: call a discovered MCP tool, list and read MCP
//! resources, authenticate to an MCP server, and trigger a remote run.
//!
//! Moved out of `lib.rs` as one domain (DEV-STRUCT-01). Bodies are verbatim.
//!
//! The registry these handlers read and write is process-global and lives in
//! [`crate::registries`].

use serde::Deserialize;
use std::time::Duration;

// The blocking client, as in `web_fetch`: these tools run on the tool thread.
use reqwest::blocking::Client;
use serde_json::{json, Value};

use super::to_pretty_json;
use crate::registries::global_mcp_registry;
