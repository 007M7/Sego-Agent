...
| `unenforced` | The check fails hard when it runs, but nothing requires it to pass before a merge. Nothing carries this status today: `main` is protected and requires `ci-ok`. |
| `merge-gated` | The check fails hard when it runs, and a merge is blocked until it succeeds - through the `ci-ok` aggregator that branch protection requires. |
...
| Cross-platform CI job | `.github/workflows/rust-ci.yml` `test-platforms` runs `cargo test --workspace` on a windows/macos matrix, with Python provisioned so the interpreter-dependent fixtures actually run rather than skip. | `merge-gated` | Runs exist and are green on `main`. A red run blocks a merge through the merge-gating row below. |
| Contract metadata job | `.github/workflows/rust-ci.yml` `contracts` validates the `schema/*.json` `x-contract-*` metadata. | `merge-gated` | It checks that the contract headers are present and consistent. It does not check that the implementation matches the schema. |
| Dependency audit job | `.github/workflows/rust-ci.yml` `dependency-audit` runs `cargo deny check` and is **not** advisory: findings must be fixed or waived with a written reason in `rust/deny.toml`. | `merge-gated` | It reads the remote advisory database, so a newly published advisory can fail it without any repository change - intended, not a flake. It does not prevent a merge (see the merge-gating row), and it audits the resolved graph, not the code that uses it. |
| Dependency waivers | `rust/deny.toml` currently waives two advisories, both unmaintained-not-vulnerable crates arriving through `syntect`; `runtime/tests/dependency_policy.rs` fails if any waiver lacks a reason. | `evidenced` | A waiver is a recorded decision, not a fix. Replacing `syntect` is the only real removal and is a product decision. |
...
| Coverage floor | `.github/workflows/rust-ci.yml` `coverage` job runs `cargo llvm-cov --workspace --fail-under-lines 75`; `runtime/tests/coverage_policy.rs` asserts the CI number equals the recorded one and that every crate has a floor. | `merge-gated` | A ratchet against a drop, not a quality target. The floor is below the measured value on purpose because CI measures on a platform where more code compiles. |
| Clippy workflow | `.github/workflows/rust-ci.yml` runs `cargo clippy --workspace --all-targets -- -D warnings`, as a gate with no `continue-on-error`; `AGENTS.md` and `CONTRIBUTING.md` require the same command of a contribution. | `merge-gated` | It fails hard when it runs and blocks a merge (see the merge-gating row). Its view is also platform-dependent: a lint behind `#[cfg(not(windows))]` is invisible to a Windows machine and vice versa, so "passes locally" and "passes in CI" are not the same claim. |
...
| Merge gating | `main` requires `ci-ok` (protection enabled 2026-09-18), and a pull request is required, with no approving review required - a single maintainer cannot approve their own. `enforce_admins` is off, so an owner retains a bypass for an outage. | `evidenced` | A required check is only as strong as what it aggregates: `ci-ok` fails if any job it depends on did not succeed, verified by breaking a test and watching it fail. It does not cover a check that never runs, which is why the path filters were removed - a required check on a filtered workflow would leave a pull request waiting on one that never reports. |
